...
export default class ObTtsPlugin extends Plugin {
    ...
    async textToSpeech(editor: Editor, text: string) {
        ...
        try {
            ...
            const customPlayer = `<button data-audio-id="${audioId}" style="border-radius:10px; height:24px; background-color:#f3f9f1; padding:0 8px;">▷</button><audio id="${audioId}" src="${fullPath}" hidden></audio>`;
            editor.replaceSelection(`${text} ${customPlayer}`);
    ...
    document.addEventListener('click', (event) => {
      ...
      if (target.tagName === 'BUTTON' && target.dataset.audioId) {
        const audioElement = document.getElementById(target.dataset.audioId) as HTMLAudioElement;
        if (audioElement) {
          audioElement.play();
        }
      }
